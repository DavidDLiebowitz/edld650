...
ell %>% group_by(elprgm) %>% summarise (n_students= n(), mean_tchr_prcpt_m = mean(tmathk), mean_tchr_prcpt_l = mean(tlangk))
...
#Math
raw_diff_m <- ell %>% group_by(elprgm) %>% summarise(n_students = n(), mean_math=mean(tmathk), se_math = sd(tmathk)/sqrt(n_students))
...
#Reading
raw_diff_e <- ell %>% group_by(elprgm) %>% summarise(n_students = n(), mean_read=mean(tlangk), se_read = sd(tlangk)/sqrt(n_students))
...
descriptives <- tableby(EL ~ prelas + ebrs + kread + kmath + ses + female + rural + hisp, numeric.stats=c("meansd"), cat.stats=c("N", "countpct"), digits=2, data=ell)
#create labels
