...
p_load(here, haven, tidyverse, ggplot2, ggthemes, fixest, modelsummary, kableExtra, broom, stargazer)
...
treat <- ggplot() + geom_point(data=subset(full_data, year==2005), aes(norm, receive_williams),
          ...
          geom_vline(xintercept = 0, color = slate, size = 1.5, alpha = 0.5) +  
...
bunch <- ggplot() + 
         ...
         geom_vline(xintercept = 0, color = slate, size = 1, alpha = 0.5) +  
         theme_pander(base_size = 12) + 
         scale_x_continuous("API rank, centered at intended cut-off for Williams (643)") +
...
###Could also do it with binned data
