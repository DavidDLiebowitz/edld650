...
p_load(here, tidyverse, DT, ggplot2, xaringan, knitr, kableExtra, modelsummary, stargazer, xaringanthemer, gganimate, ggthemes, fixest, haven, broom)
...
```{r, echo=FALSE, eval = T}
DT::datatable(dynarski[, c(1, 4:8)], fillContainer = FALSE, options = list(pageLength = 7))
```
...
dd <- ggplot(dynarski, aes(post,coll)) + aes(color=factor(fatherdec)) + 
      ...
      color="Father Deceased") + scale_x_continuous(name="Year", 
      breaks=c(0, 1), limits=c(-0.25, 1), labels=c("0"= "pre-1981", "1"= "post-1981")) +
      expand_limits(y=c(0.35, 0.6), x=c(-0.25, 1)) +
      annotate("text", x = 0.75, y = 0.45, label = "NOT deceased father", color = "#F8766D", size = 6) +
      annotate("text", x = 0.75, y = 0.39, label = "DECEASED father", color = "#00BFC4", size = 6) +
...
#### Research Project Proposal due 11:59pm, 4/21
- Talk to me!
