...
```{r, echo=F, fig.height=5.5}
first_cut <- filter(maimonides, size>=29 & size<=53)

bunch <- ggplot() +
            geom_vline(xintercept = 40.5, color = slate, size = 1.5, alpha = 0.5) +       
            stat_count(data=first_cut, aes(size), fill=blue, geom="bar") +
            theme_pander(base_size = 18) + 
...
```{r, echo=F, fig.height=5.5}
sort <- ggplot() +
  geom_boxplot(data=first_cut, aes(x=as.factor(size), y=ses), 
...
```{r, echo=F, fig.height=5.5}
quantile <- ggplot() +
  geom_quantile(data=filter(first_cut, size<41), aes(size, ses), quantiles=0.5, color=purple) + 
  geom_quantile(data=filter(first_cut, size>=41), aes(size, ses), quantiles=0.5, color=red_pink) +
  geom_vline(xintercept = 40.5, color = slate, size = 1.5, alpha = 0.5) +
  theme_pander(base_size = 18) +
...
```{r, echo=F, fig.height=5.5}
fx <- ggplot() +
  geom_point(data=first_cut, aes(x=size, y=read), color=blue, alpha=0.8, shape=16) +
  geom_vline(xintercept = 40.5, color = slate, size = 1.5, alpha = 0.5) +
  theme_pander(base_size = 18) +
...
```{r, echo=F, fig.height=5.5}
first_cut <- mutate(first_cut, small = ifelse(size >= 41,TRUE,FALSE))

fx2 <- ggplot() + 
  geom_point(data=first_cut, aes(x=size, y=read, colour=small), alpha=0.8, shape=16) + 
  geom_vline(xintercept = 40.5, color = slate, size = 1.5, alpha = 0.5) +
...
```{r, echo=F, fig.height=5.5}
binned <- first_cut %>% group_by(size) %>% summarise_at(c("read", "small"), mean)

binned_plot <- ggplot() + 
  geom_point(data=binned, aes(x=size, y=read, colour=as.factor(small)), alpha=0.8, shape=16, size=3) + 
  geom_vline(xintercept = 40.5, color = slate, size = 1.5, alpha = 0.5) + 
  theme_pander(base_size = 18) + xlab("Cohort size") + ylab("Verbal score") + 
  scale_color_manual(values = c(purple, red_pink), name="Small") +
...
lm_tmp <- lm(read ~ size + I(size>40.5), data = first_cut)
...
```{r, echo=F, fig.height=5.5}
linear_fx <- fx2 + 
    geom_smooth(data=first_cut, aes(x=size, y=read, colour=small), method = lm, se=FALSE)
...
```{r, echo=F, fig.height=5.5}
linear_fx_band <- fx2 + 
  geom_smooth(data=filter(first_cut, size>=36 & size<=46), aes(x=size, y=read, colour=small), method = lm, se=FALSE)
...
```{r, echo=F, fig.height=5.5}
linear_fx_se <- fx2 + 
  geom_smooth(data=first_cut, aes(x=size, y=read, colour=small), method = lm, se=TRUE)
...
```{r, echo=F, fig.height=5.5}
non_linear_fx <- fx2 + 
  geom_smooth(data=first_cut, aes(x=size, y=read, colour=small), method = lm, formula=y ~ poly(x,2), se=TRUE)
...
linear_const <- lm(read ~ size + I(size>40.5), data = first_cut)
linear_diff <- lm(read ~ size * I(size>40.5), data = first_cut)
quadratic <- lm(read ~ poly(size,2) + I(size>40.5), data = first_cut)

modelsummary(list(linear_const, linear_diff, quadratic), stars=T)
#stargazer(linear_const, linear_diff, quadratic, type='html', omit.stat = c("ser", "adj.rsq", "f"), single.row=T, dep.var.caption="", dep.var.labels.include=F, intercept.bottom=F, star.cutoffs=c(0.05, 0.01, 0.001), notes.align="l")
```
---
...
# Just for fun...
```{r, echo=F, fig.height=5.5}
fx2 + geom_smooth(data=first_cut, aes(x=size, y=read, colour=small), 
...
## Week 5: Regression Discontinuity
