...
- .large[**Conclusion**]: But really `r fontawesome::fa("smile-wink")`, X causes Y.
...
dag3a <- ggdag::dagify(y ~ d,
                       d ~ y,
                     exposure = "d",
                     outcome = "y",
                coords = list(x = c(d = 1, y = 2),
                              y = c(d = 2, y = 3))
                              )
ggdag::ggdag_status(dag3a, text_size = 8) +
    guides(fill = F, color=F) +
    ggdag::theme_dag() 
```
...
ggdag::ggdag_status(dag3b, text_size = 8) +
    guides(fill = F, color=F) +
    ggdag::theme_dag() 
```
...
animate(p,nframes=200, renderer = gifski_renderer(loop = FALSE, height=450))
```
...
# create the brace
make_brace <- function(x, y0, y1, width = 0.05, n = 100) {
...
data %>% 
  ...
  geom_path(
    ...
    size = 1,
    color = "black") +
  annotate("text", x = 2.35, y = 4.65, label = "Treatment effect") +
...
- Project proposal by April 21
  - Meet w/ teaching staff to discuss at least once
  - In class scholarly presentation (June 3)
  - Written final research project (June 11; *optional feedback by June 5*)
]
...
### Assignments Due
- Complete student survey on Canvas (April 3)
- DARE #1 due: 11:59pm April 14
