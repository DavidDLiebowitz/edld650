...
With 10 minutes you have time for:
1. What is the focus and why is this important? (1 slide)
2. What are the research questions? (1 slide)
3. What are the key features of the sample and data (1-2 slides)
4. What is the methodology and research design? (2-3 slides)
5. What is the result? (3 slides)
...
ex <- data.frame(pre_x, post_x, pre_y, post_y)

# create the brace
make_brace <- function(x, y0, y1, width = 0.1, n = 100) {
  
  y_mid <- (y0 + y1) / 2
  
  # upper half
  t1 <- seq(0, 1, length.out = n/2)
  upper <- data.frame(
    x = x + width * sin(pi * t1),
    y = y1 - (y1 - y_mid) * t1
  )
  ...
  # lower half
  t2 <- seq(0, 1, length.out = n/2)
  lower <- data.frame(
    x = x + width * sin(pi * (1 - t2)),
    y = y_mid - (y_mid - y0) * t2
  )
...
.pull-right[
$$\hat{\beta}_{1}^{IVE} = \frac{S_{YZ}}{S_{DZ}}$$
...
--
Each research design is the best design and has the highest degree of internal validity, *given a particular data-generating process*.
...
--
An RCT with great internal validity might not teach us as much as a more generalizable RD in which there are multiple discontinuities in an authentic (no *founder effects*) setting.

--

Always conduct research designed to make causal inferences with a mix of "humble and hotshot" attitude
...
## Week 10: Presentate! `r fontawesome::fa("smile")`
...
### Readings: 
- **If haven't yet**: *MM* Ch. 13 and 14
